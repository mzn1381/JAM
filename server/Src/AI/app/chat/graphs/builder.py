...
from app.chat.nodes.intent_router_node import intent_router_node
from app.chat.nodes.action_navigator_node import action_navigator_node
...
from app.chat.nodes.rag_node import rag_node
...
def route_intent_jam(state: GraphState) -> str: ## should be refactored  #MGZ!!!
    if not state.intent:
        return "chitchat"
    ...
    if category == IntentCategory.REGISTERY_ARAD:
        return "registery_arad"
    if category == IntentCategory.RAG:
        return "rag"
...
def build_graph(llm):
    ...
    workflow_jam.add_node("intent_router",  lambda s: intent_router_node(s, llm)) ## prompt template for is rag or not 
...
#######################################
    ...
    workflow_jam.add_edge("intent_router", "history_recorder")
    ...
    return workflow_jam.compile(checkpointer=get_checkpointer())
