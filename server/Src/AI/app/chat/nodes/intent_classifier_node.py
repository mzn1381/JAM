...
class _IntentClassifierStructuredOutput(BaseModel):
...
def _resolve_category(result: _IntentClassifierStructuredOutput) -> tuple[str, float, str]:
...
def intent_classifier_node(state: GraphState, llm) -> GraphState:
    ...
    result = llm.complete_structured_output(
        ...
        _IntentClassifierStructuredOutput,
    )
