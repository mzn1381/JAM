...
برای دریافت توکن مورد نیاز جهت کال کردن سرویس چت، باید از پنل [ادمین](https://web.mypishkar.ir/api-keys) استفاده شود.
...
آدرس پایه برای فراخوانی endpoint چت مقدار زیر است:
...
```http
POST https://ai.mypishkar.ir/api/v2/Chat
```
...
```bash
curl -X POST "https://ai.mypishkar.ir/api/v2/Chat" \
